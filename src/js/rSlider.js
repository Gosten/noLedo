(function () {
	...
	RS.prototype.createScale = function (resize) {
		...
		for (var i = 0, iLen = this.conf.values.length; i < iLen; i++) {
			...
			if (!this.conf.labels) {
				if (i === 0 || i === iLen - 1) ins.innerHTML = mapGrade(this.conf.values[i])
			}
			else ins.innerHTML = mapGrade(this.conf.values[i]);
	...
	RS.prototype.setValues = function (start, end) {
		...
		if (this.conf.range) {
			if (this.conf.tooltip) {
				this.tipL.innerHTML = mapGrade(this.conf.values[this.values.start]);
				this.tipR.innerHTML = mapGrade(this.conf.values[this.values.end]);
			}
			...
			if (this.conf.tooltip)
				this.tipL.innerHTML = mapGrade(this.conf.values[this.values.end]);
	...
	//changed timeout from 500ms to 0ms
